...
> **Note:** this file assumes `project RULES doc` (project root) is loaded as a persistent Rule, not re-read from this doc. That file carries the Specificity Protocol, target-hardware/performance budgets, Day-0 risk spikes, and the repo-hygiene rules — all of it applies to M0 even though it isn't repeated below. Ensure the project RULES doc is loaded before starting M0 work; do not guess at budgets or scope.
...
#### [M0-EXT-15] Game Draw Path Bypasses Cull/Indirect (demo-only)
...
#### [M0-EXT-16] Hardcoded 100-Entity Cap
...
#### [M0-EXT-17] Hi-Z Depth Pyramid Has No Producer
...
#### [M0-EXT-18] Orphaned Staging / Frame-Deletion Systems
...
#### [M0-EXT-19] No Async Shader Compile + Watchdog
...
#### [M0-EXT-20] SoA Hot-Component Layout
...
#### [M0-EXT-21] Meshlet / Mesh-Shader Pipeline
...
#### [M0-EXT-22] Parallel Secondary Command-Buffer Recording
...
#### [M0-EXT-23] Occlusion Query Double-Buffering
...
#### [M0-EXT-24] Parallel-Deterministic PRNG Core
...
#### [M0-EXT-25] Per-Fiber Exception Containment
...
#### [M0-EXT-26] Single-Instance Boot Lock
...
#### [M0-EXT-27] Frame-Deletion Queue Wiring
...
#### [M0-EXT-28] GPU Validation in Release Builds
...
#### [M0-EXT-29] Release Build Sanitizer Path
...
#### [M0-EXT-30] Descriptor-Set Fragmentation Guard
...
#### [M0-EXT-31] Secondary CB on Game Path
...
#### [M0-EXT-32] Platform Support (Linux/macOS/console)
...
#### [M0-EXT-33] Build & Packaging Pipeline
...
#### [M0-EXT-34] Automated Test Matrix (unit/integration)
...
#### [M0-EXT-35] Performance Budget & Profiling Dashboard
...
#### [M0-EXT-36] Anti-Tamper & Memory Integrity
...
#### [M0-EXT-37] Crash Reporter & Minidump Upload
...
#### [M0-EXT-38] Analytics & Privacy-Consent
...
#### [M0-EXT-39] Capability-Tier Runtime Fallback

##### Systems Touched
M0-EXT-01 tiering + M0-EXT-40 dynamic res + M4.6 cull.

##### Math
if(!Feature(x)) Enable(Fallback(x)); quality = min(tier, hw)

##### How It Works
At boot, probe supported GPU/API features; if a tier feature unavailable, auto-drop to next-lower implementation (e.g. no mesh shading -> standard draw) instead of crashing. Works with M0-EXT-40 to keep 6GB floor stable.

##### Reference Implementation
```cpp
if(!Supports(feature)) Use(fallback[feature]);
```

##### Player-Facing Impact
Game degrades gracefully on weak hardware instead of failing to launch.
