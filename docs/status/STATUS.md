...
4. ~~**M0:step33** — Deterministic crash-repro via replay: mechanism present (record/replay + tick-hash, `Input.cpp`/`Engine.cpp`), but replay-identity (identical tick traces across runs) was **not re-verified live** this pass. Needs a crash+replay pair run on real GPU.~~ (deferred to hardware run)
5. ~~**M0-EXT07** — Spec block exists (M0.md:290); **investigated 2026-07-20**: enkiTS v1.12 does NOT include fiber-support APIs (`WaitForTaskSet`/`SetFiber` absent from TaskScheduler.h). Implementating custom fiber yield would require either (a) upgrading enkiTS to a fiber-capable version or (b) building Windows-fiber wrappers (`ConvertThreadToFiber`/`SwitchToFiber`). Both consumers (M2-EXT-01, M4-EXT-03) are not yet built, so this is deferred. **Not a code gap — a discovery.**~~
6. ~~**M0-EXT08** — Spec block exists (M0.md:315); **IMPLEMENTED 2026-07-20**: `src/render/DescriptorSlotAllocator.h/.cpp` — 16-page bitmask free-list, 1024-slot bindless storage handle allocator, round-robin hint, double-free guard. Compiled, linked, standalone-tested (exit 0). Full build: [19/39].~~
7. ~~**M0-EXT10** — Spec block exists (M0.md:367); **IMPLEMENTED 2026-07-20**: `src/render/PipelineCompatValidator.h/.cpp` — validates SPIR-V reflected binding descriptors + push-constant ranges against VkPipelineLayout (Vulkan 1.4). Uses spirv-reflect. Compiled, linked. Full build: [27/39].~~
...
## Last Verified Build
- Branch: `main`
- Master Refactor Plan: **198 concrete features across 6 Plain-English Stages + 4 Console Ports (v8.2)** (verified 0-cycle DAG, 0 dangling, 100% mirror parity via `scripts/verify_plan_v8.py`).
...
- Graphics & Upscaling: **Vulkan 1.4 Native Core** (`VK_KHR_dynamic_rendering_local_read`, `VK_KHR_push_descriptor`, `VK_KHR_maintenance5/6/9`, `VK_EXT_descriptor_buffer`, `VK_EXT_shader_object`, `VK_EXT_hdr_metadata`), **AgX Color Science** tonemapping, **Easy Automatic HDR** (Auto-negotiated 10-bit HDR10 PQ / scRGB FP16 with zero calibration, display metadata injection, and paper-white UI protection), and multi-vendor neural upscaler integration (**NVIDIA DLSS 4.5** with Multi-Frame Generation & Transformer SR, **AMD FSR 4**, **Intel XeSS 2.0+**).
