...
* **Procedural Generation — World/Systems** (Poisson disk sampling, domain-warped noise, WFC adjacency validator, chunk-boundary determinism test, graph-based room connectivity, rejection sampling, seed-sharing, cellular automata, graph grammars, genetic algorithms, Whittaker biome classification, thermal erosion, spline-based roads/rivers, hidden difficulty correction) — deferred, engine layer first.
* **Procedural Asset Generation (Solo/No-Artist)** (histogram-preserving blending, PPTBF materials, node-graph procedural materials, noise→normal map derivation, procedural mesh via primitive+grammar, procedural foliage L-system, procedural color palette, procedural UV unwrap, procedural skeletal rig generation, procedural decal atlasing, procedural loot icon generation) — deferred, engine layer first.
...
* **Procedural Mission & Event Director** — generates encounters/caches/rescue tasks from local socio-economic + SEIR tags.
