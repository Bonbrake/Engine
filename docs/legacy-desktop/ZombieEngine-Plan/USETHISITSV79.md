# Prompt for Google Antigravity 2.0 — "The Endless Quarantine": Open-World Zombie Survival Engine, Full Rebuild M0 → M13 (v77)
...
* M2.6E — Creation Engine editor (cell painter, kit browser, navmesh bake, leveled lists, keywords, dialogue graph, Lua scripts, preview-in-engine)
...
* M13 — Local Small Language Model (SLM) integration — core system: **twenty** always-on `M13-EXT` generators feed mission text, world lore, journals, signage, faction radio, companion barks, quest rumors, terminal logs, holotape scripts, radiant flavor, settlement broadcasts, faction map legends, and player legend across M4/M5.4/M6/M8/M8.5/M8.6/M10/M11; not a bolt-on flavor pass
...
*New: M2.6E (Creation Engine editor) contains its own Extended Systems Library with editor-specific `[M2.6E-EXT-NN]` blocks.*
...
**8. No editor exists.** All world authoring, leveled list editing, keyword management, dialogue/quest graphing, and script attachment must happen in-engine via a Creation Engine–class tool (M2.6E).
**9. No seamless interior/exterior streaming.** Interiors currently require loading screens; the spec needs unified World Partition streaming with async door preload (<200ms).
**10. Karma is single-axis.** Fallout:NV/Gothic/Tyranny prove multi-axis karma + per-faction reputation drives deeper reactivity.
**11. Companions lack personality-driven behavior.** Dragon's Dogma inclinations + BG3 personal quests + FNV command wheel = deeper companions.
...
6. **AUTHORED WHERE IT MATTERS, PROCEDURAL WHERE IT SCALES.**
   Landmarks, quests, NPCs, voices, key encounters = YOURS.
   Rocks, trees, clutter, loot rolls, interior furnishing, ambient text = ENGINE'S.

7. **SEAMLESS EVERYTHING — NO LOADING SCREENS AFTER INITIAL BOOT.**
   Unified World Partition streaming for exteriors + interiors. Async door preload (<200ms).
   Portal-based occlusion. Data Layers for quest/time/faction conditional content.

8. **MODDER-FIRST ARCHITECTURE.**
   Every runtime system editable in-editor (M2.6E). JSON plugin format (`mods/<mod>/` priority chain).
   AssetPath resolver: `mods/<mod>/` > base game. No .esp/.esm parser needed — JSON is mergeable, version-controllable, human-readable.
...
* **Editor queue-family reservation:** During M0 device creation, explicitly enumerate and reserve a dedicated compute queue family (VK_QUEUE_COMPUTE_BIT without VK_QUEUE_GRAPHICS_BIT) for the M2.6E preview renderer and M13 SLM worker. If no dedicated family exists, fall back to shared graphics+compute family rather than failing init.
...
*New M0 EXT blocks for editor/SLM infrastructure:*
...
#### [M0-EXT-11] Editor Compute Queue Reservation

##### Systems Touched

M0 device creation, M2.6E preview renderer, M13 SLM worker thread.

##### Math

Dedicated compute queue family index `q` where `(queueFlags & VK_QUEUE_COMPUTE_BIT) != 0` and `(queueFlags & VK_QUEUE_GRAPHICS_BIT) == 0`. Fallback: graphics family index.

##### How It Works

During M0 device creation, explicitly enumerate and reserve a dedicated compute-only queue family for the M2.6E preview renderer (renders editor viewport) and M13 SLM worker thread. If no dedicated family exists, fall back to shared graphics+compute family rather than failing init. This queue is created with `VK_QUEUE_COMPUTE_BIT` only and used exclusively for editor preview rendering and SLM inference compute shaders.

##### Reference Implementation

```cpp
uint32_t FindDedicatedComputeQueueFamily(const std::vector<VkQueueFamilyProperties>& families, uint32_t graphicsFamilyIndex) {
    ...
    return graphicsFamilyIndex; // fallback: share graphics+compute family
}
```

##### Player-Facing Impact

Invisible — this is what lets M2.6E preview renderer and M13 SLM run without stealing frame time from rendering or physics, and stops the engine from misdetecting hardware and silently falling back to a slow path.
