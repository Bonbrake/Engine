...
- [ ] ZombieEngineTests link failure — **RESOLVED (2026-07-12)**. Root cause confirmed: `spawnDevTestBody_` was defined under `#if ENGINE_DEV_TOOLS` (`Engine.cpp:474`) but referenced from `mainLoop` (`Engine.cpp:248-252`, `:268-280`) and declared in `Engine.h` (`:28-32`, `:43-46`) **without** matching guards. The test target (`CMakeLists.txt:160-163`) does not define `ENGINE_DEV_TOOLS`, so the symbol compiled out of the definition but its references/declarations remained → LNK2019. **Fix:** wrapped the two `mainLoop` call sites and the `Engine.h` declaration + state members in `#if ENGINE_DEV_TOOLS` (matching the definition's guard). Result: `ZombieEngineTests` links and **ALL TESTS PASS (35 assertions, 7 cases)**; main `--dev` still spawns the dev body + fires lethal damage (feature intact, macro defined in main target). Committed separately from frame-dump / Slice 0a as its own commit on `m2/physics-destruction`.
