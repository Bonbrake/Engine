...
#### [M13-EXT-14] Offline SLM Generation Harness
...
#### [M13-EXT-15] SLM Prompt-Template Library
...
#### [M13-EXT-16] SLM Output Schema Validator
...
#### [M13-EXT-17] SLM Output → Validation Pipeline
...
#### [M13-EXT-19] SLM Deterministic Seeding
...
#### [M13-EXT-20] SLM Fallback on Failure
...
#### [M13-EXT-22] SLM Quest & Objective Text
...
#### [M13-EXT-23] SLM Dialogue & Branching
...
#### [M13-EXT-24] SLM Lore & Worldbuilding
...
#### [M13-EXT-25] SLM Loadout/Build Suggestions
...
#### [M13-EXT-26] SLM Moderation/Filtering
...
#### [M13-EXT-51] Content-Drought Guard (Living-World Cadence)

##### Systems Touched
M13 radiant + M5.4-EXT-16 ambient + M4-EXT-74 graph + SLM broadcasts + M8.6 factions.

##### Math
idle = timeSince(lastMeaningfulEvent); if(idle > cadence) Spawn(weightedEvent, nearPlayer)

##### How It Works
Open-world games go stale when player runs out of things to do. Guard tracks time since last meaningful event (mission/encounter/faction move/POI reveal), past soft cadence proactively spawns weighted context-aware event near player via world-graph + SLM - world never quiet.

##### Reference Implementation
```cpp
if(now - lastEvent > cadence) Offer(WeightedEvent(playerHistory), nearPlayer);
```

##### Player-Facing Impact
World keeps pulling you in - never left wandering an empty map.
