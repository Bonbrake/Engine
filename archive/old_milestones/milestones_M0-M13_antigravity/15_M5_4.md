...
#### [M5.4-EXT-08] Closed-Ecosystem Continuum-Fluid Horde Density Field *(RECONSTRUCTED FROM CITATION CONTEXT — VERIFY)*

##### Systems Touched
M5.4 horde motion. Simulates horde density as a continuum fluid field for smooth pressure.

##### Math
drho/dt = -div(rho*v) + diffuse; v from gradient descent of (rho + threat); advection on grid.

##### How It Works
Horde density is a continuum field advected by a velocity derived from its own gradient plus threat avoidance; pressure resolves crowding so hordes spread and flow rather than stacking. Cheaper and smoother than per-agent packing at scale.

##### Reference Implementation
```cpp
rho = Advect(rho, v, dt); v = -grad(rho) - threatGrad;
```

##### Player-Facing Impact
Hordes flow and spread like a fluid - no unnatural clumping, believable swarm motion.

---

#### [M5.4-EXT-19] Faction Caravan Long-Range Router *(RECONSTRUCTED FROM CITATION CONTEXT — VERIFY)*
