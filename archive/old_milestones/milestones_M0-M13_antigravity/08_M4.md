...
#### [M4-EXT-74] World-Graph Backbone *(RECONSTRUCTED FROM CITATION CONTEXT — VERIFY)*

##### Systems Touched
M4 + all POI/faction/encounter generators. The central web every system attaches to: nodes (region/POI/hub/safe-zone) + edges (road/tunnel/smuggle-route), weighted by distance + threat.

##### Math
node = {id, pos:vec3, type}; edge = {a,b, w = dist(a,b) * threat(b)}; path = A*(node, w). threat(b) = SEIR/conflict scalar at b.

##### How It Works
A single WorldGraph (EnTT entities per node/edge) holds region/POI/hub/safe-zone nodes and road/tunnel/smuggle-route edges; edge weight = Euclidean distance x threat multiplier at the destination. All POI/faction/encounter generators attach here; it is updated incrementally as the world streams. A* over it resolves navigation, faction turf, and PvP Contested Zones. Nothing attaches to nothing.

##### Reference Implementation
```cpp
struct WorldGraph { entt::registry nodes; entt::registry edges; };
auto p = AStar(graph, from, to, [](Edge e){ return e.w; });
```

##### Player-Facing Impact
The world is one connected organism - navigation, faction turf, and PvP contested zones all resolve through a single graph, so the map behaves coherently instead of as a pile of disconnected features.
