...
#### [M2-EXT-08] Compound-Collider Corpse Pile Merging

##### Systems Touched

Jolt physics body management, `[M2-EXT-02]`'s island-sleep `HibernationEvent` bridge and the `[M7-EXT-01]` corpse-flatten/hibernation pipeline it feeds, M1's `SpatialHash` (per-cell corpse occupancy), enkiTS worker pool, `[M2-EXT-01]`'s async shape-swap discipline.

##### Math

A spatial cell triggers a merge when its resident flattened-corpse count crosses a density threshold:

`Merge(cell) ⟺ Count_flattenedCorpses(cell) > D_merge` (default `D_merge = 8`, tuned against the siege/kill-zone case)

Body-count reduction per merged cell: `N → 1` (N constituent static hulls collapse into one `JPH::StaticCompoundShape`).

##### How It Works

`[M2-EXT-02]`'s bridge already fires `HibernationEvent` when a ragdoll settles, and `[M7-EXT-01]` flattens settled corpses to static hulls. This feature watches per-cell flattened-corpse occupancy through the same `SpatialHash` cell the corpses already register in. When a single nav/spatial cell accumulates more than `D_merge` flattened static hulls — the long-siege or fixed kill-zone scenario where bodies stack in one place — it dispatches an enkiTS worker to build one `JPH::StaticCompoundShape` composed of the constituent hulls' shapes at their baked-in relative transforms, keyed to that cell. The merge is the same fail-safe atomic pattern as `[M2-EXT-01]`: the compound is cooked on the background thread, then a single main-thread step adds the one merged static body via `JPH::BodyInterface` and removes the N constituent bodies, so the physics world never observes a half-merged state. Because the inputs are already static, sleeping, flattened corpses (no active simulation, no gameplay interaction beyond being walked/shot over), replacing N bodies with 1 changes nothing observable — it only collapses N broad-phase entries and N narrow-phase proxies into one. Merged cells are keyed so that if the pile is later disturbed past a re-activation condition (e.g. explosion in-cell), the merged body can be removed and its constituents restored from their retained descriptors rather than lost. Merges are rate-limited per frame so a mass-death event doesn't dispatch hundreds of cook tasks in one tick.

##### Reference Implementation

```cpp
// Runs on an enkiTS worker once a cell's flattened-corpse count exceeds the threshold; the returned
// compound is added (and the constituents removed) in a single atomic main-thread step, matching
// [M2-EXT-01]'s shape-swap discipline. Never merges non-static or awake bodies.
JPH::ShapeRefC CookCorpsePileCompound(const std::vector<CorpseHull>& hulls) {
    JPH::StaticCompoundShapeSettings settings;
    for (const auto& h : hulls) {
        // h.shape is the already-cooked flattened static hull; h.relPos/h.relRot are its transform
        // relative to the pile cell origin (kept single-precision — corpses are near-origin static geometry)
        settings.AddShape(h.relPos, h.relRot, h.shape);
    }
    JPH::Shape::ShapeResult result = settings.Create();
    return result.IsValid() ? result.Get() : JPH::ShapeRefC{}; // caller keeps constituents on failure
}

inline bool ShouldMergeCell(uint32_t flattenedCorpseCount, uint32_t mergeThreshold /*= 8*/) {
    return flattenedCorpseCount > mergeThreshold;
}
```

##### Player-Facing Impact

Invisible directly — during a long siege where hundreds of bodies pile up in a defended chokepoint, the physics body count stops climbing linearly with the kill count, protecting the physics frame budget at exactly the moment the defense is most intense. No visual change: the pile looks and behaves identically, it's just one collider underneath instead of many.

##### Depends on (not yet built at this point)

M7 — `[M7-EXT-01]`'s corpse-flattening produces the static hulls this merges; implement/verify against `[M2-EXT-02]`'s hibernation flag for now and wire the M7 flatten output once that milestone lands.
...
**Once Exit Criteria pass:** load the next milestone file in sequence: `03_M2_6.md` (M2.6).
...
#### [M2-EXT-09] Character Controller / Player Movement
...
#### [M2-EXT-11] Ballistics / Projectile Resolution
...
#### [M2-EXT-12] Destructible = Placeholder Swap Only
...
#### [M2-EXT-14] Hit/Limb Resolution Pipeline
...
#### [M2-EXT-15] Physics Debug Draw (shipping toggle)
...
#### [M2-EXT-16] No Real Content / Player / Zombie
...
#### [M2-EXT-19] Survival Physiology (stamina/hunger)
...
#### [M2-EXT-20] Crafting / Recipes / Workbench
...
#### [M2-EXT-21] Skills / Perks / Progression
...
#### [M2-EXT-22] Procedural Variation (bio-weight)
...
#### [M2-EXT-23] Physics Determinism Test
