...
#### [M6.5-EXT-13] G-Buffer Decal Normal Blending

##### Systems Touched

M6.5 decal/projection system (`[M6.5-EXT-09]` linked-list OIT / `[M6.5-EXT-07]` depth-buffer particle proxies provide the decal geometry & ordering), M4.5 G-buffer normal attachment (the target this writes into), `[M4.5-EXT-22]`'s glass-fracture normal perturbation (same normal-write stage, different source).

##### Math

A decal's normal `n_dec` is blended into the G-buffer surface normal `n_gb` with a per-decal strength `s ∈ [0,1]` (the projection's coverage/opacity), typically as a normalized lerp or a Whiteout/reoriented blend to avoid the flattening artifact of a naive average:

`n_blend = normalize(lerp(n_gb, n_dec, s))`  (or `normalize(n_gb + s·(n_dec − n_gb))` for the tangent-space variant)

Multiple overlapping decals accumulate by blending in draw order, each against the running G-buffer normal.

##### How It Works

Decals (blood spatter, scorch marks, bullet gouges, the `[M6.5-EXT-09]` projected forensic skin/tissue decals) need to perturb the lit surface normal where they land, not just tint albedo, or they read as flat stickers pasted on geometry. As each decal is projected (its footprint resolved against the depth buffer via `[M6.5-EXT-07]`'s proxy), this EXT blends its tangent-space normal into the G-buffer normal attachment with the decal's coverage strength, so a blood splatter adds micro-relief that catches light and a scorch mark reads as scorched, not painted. The blend strength is the decal's own opacity/coverage, so a partial decal perturbs proportionally; overlapping decals accumulate in draw order, each refining the running normal. It writes in the same normal-write stage M4.5 already owns (reusing `[M4.5-EXT-22]`'s path) and composes with the wetness/saturation shading (`[M4.5-EXT-06]`) so a wet decal still reads correctly. No new render pass — it's one more writer to the existing G-buffer normal target during the decal projection pass.

##### Reference Implementation

```cpp
// Blends a decal's tangent-space normal into the G-buffer normal with the decal's coverage strength.
// Runs in the existing normal-write stage (reuses [M4.5-EXT-22]'s path). Overlapping decals accumulate.
glm::vec3 BlendDecalNormal(const glm::vec3& nGBuffer, const glm::vec3& nDecalTS, float strength) {
    return glm::normalize(glm::mix(nGBuffer, nDecalTS, std::clamp(strength, 0.0f, 1.0f)));
}
```

##### Player-Facing Impact

Decals look stamped into the surface rather than pasted on top — blood spatter and scorch marks catch the light with real relief, bullet gouges read as pitted metal, and overlapping marks layer believably instead of z-fighting or floating. The world's damage and grime storytelling holds up under close inspection and dynamic lighting.


---

## Appendix Cross-References for M6.5

*The following items from the doc's Appendices/§5.x Addenda explicitly reference M6.5 in their "Systems Touched" or scope. Auto-matched by milestone ID — verify relevance before feeding to Antigravity.*
...
**Once Exit Criteria pass:** load the next milestone file in sequence: `18_M7.md` (M7).
...
#### [M6.5-EXT-16] Weather Particle VFX (rain/snow)
...
#### [M6.5-EXT-17] Death/ Dissolve VFX
