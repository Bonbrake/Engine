...
> **ctx** · Beer-Lambert Translucent Medium Light Extinction -- Lighting/translucency ([M4.5]), shaders (HLSL+DXC). Depends on [M3].
...
##### Math
Beer-Lambert `T(d)=exp(-σ_t·d)`; subsurface `L_out=L_in·T + scatter·(1-T)`.
##### How It Works
1. March light through medium. 2. Attenuate by exp(-σ_t·d). 3. Add forward-scatter. 4. HLSL 1-2 sample approx for perf.
##### Reference Implementation
```cpp
float T=exp(-sigmaT*d); float Lo=Li*T+scatter*(1.f-T);
```
##### Player-Facing Impact
Flesh/glass bleed light, deep parts go dark — sells horror without perf hit (Tier-0 bounded).
<a id="M4.5-EXT-21"></a>
#### [M4.5-EXT-21] RVT Feedback Buffer & Cache Invalidation
> **tags** · RVT
