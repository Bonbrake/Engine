...
2. **Allocation fragmentation:** don't allocate a fresh device-memory block per chunk on stream-in — that fragments the pool and stalls the driver. Pre-allocate a large uniform VMA block pool at boot and sub-allocate RVT page slots linearly out of it (the same pattern as [M4-EXT-84]'s vertex-buffer pooling), returning slots to the async tracking loop on chunk eviction.
...
##### How It Works
1. Surface shade with Burley/Kalisto diffuse (NOT Lambert) — fixes the "plastic" Unreal look Threat Interactive documents. 2. For translucent media (flesh, glass, fog) march light, attenuate by `exp(-σ_t·d)`, add forward-scatter. 3. HLSL: Burley is ~1 extra mad; Beer-Lambert is 1 exp. Both Tier-0 safe.
4. **T1 (filtered-mip BRDF):** Material BRDF samples use trilinear + anisotropic filtering; roughness drives mip LOD bias so specular highlights don't alias at distance (Lazarov 2015 "specular AA" via roughness-mip-footprint — screen-space derivative of roughness used to bias mip). Combats specular aliasing at the geometry/specular level without TAA-smear.
5. **T12 (parallax occlusion mapping):** BRDF samples use POM in the base pass: ray-march the height field along the view vector in the pixel shader, offset UVs per layer. Detail on top of existing geometry — no extra depth prepass, no vertex displacement. Step count bounded (cheap, base-pass only); combines with T1 filtered-mip so parallax samples don't alias at distance.
##### Reference Implementation
```cpp
// Burley/Kalisto diffuse (Threat Interactive gospel: never ship Lambert)
float Fd90 = 0.5f + 2.f*rough*rough*pow(vh,2);
float f_d = (1.f/PI)*(1.f+(Fd90-1.f)*pow(1.f-nl,5))*(1.f+(Fd90-1.f)*pow(1.f-vh,5));
// Beer-Lambert extinction (transmission only)
float T = exp(-sigmaT*d); float Lo = Li*T + scatter*(1.f-T);
```
##### Player-Facing Impact
...
> **ctx** · GBuffer Base-Color Full-Precision Mandate -- pairs with [M4.5-EXT-20] (Burley
      shading). Prevents TI-documented "specular instability" from mid-stage precision loss.
##### Systems Touched
...
2. GBuffer normal buffer written RGB10A2 — 10-bit/channel, 2-bit for material tag in alpha.
       Higher precision than RGBA8 normals, no banding on smooth surfaces; cheap (32-bit).
...
// Normal: RGB10A2 (TI T4 — 10-bit/channel, 2-bit material tag in A)
layout(rgb10_a2) uniform image2D gNormal;
```
##### Player-Facing Impact
...
3. Dense alpha-tested/quad-overdraw geo minimized in prepass (TI: Crysis 3 dense alpha-tested
       prepass = bad; HL Alyx omits it).
##### Reference Implementation
```cpp
...
**T9 (noise-free VT mandate, TI R9 — append).** RVT page streaming MUST be noise-free: no shimmering or trudging tile pops. Page resize/eviction uses hysteresis + STBN dither (reuse K-EXT-22) so tile transitions are imperceptible — never raw nearest-pop. If feedback shows >N tile-pops/frame on Tier-0, auto-degrade to the non-VT material path (BC1/BC4 packed, M4-EXT-90) rather than ship noisy VT. TI: "Borderlands 4 cost tripled by unoptimized materials and noisy virtual textures" — noisy VT is a hard fail, not a tuning target.
...
> **tl;dr** · Skin and hair are FORWARD-rendered (opaque world = tiled deferred). VERIFIER:
  M4.5-EXT-20 (Burley/Beer-Lambert) has NO forward skin/hair pass. Genuine gap.
...
##### How It Works
1. Skin/hair meshes routed to a dedicated FORWARD pass (opaque world = tiled deferred).
   Forward pass shades directly to the lit buffer — NO GBuffer albedo/normal write.
2. Forward skin = Burley + SSS LUT (T8) computed inline.
3. Hair = alpha-tested forward, drawn after opaque, depth-sorted; minority screen (~4%).
##### Reference Implementation
```cpp
if (mat.isSkin || mat.isHair) { out = ShadeForward(p); }
```
##### Player-Facing Impact
...
> **tl;dr** · Ship a proper filmic (ACES/AgX-style) tonemapper as the default; NOT the engine
  default and NOT a "blurry filmic filter" used to hide shading errors. Tonemap is crisp.
> **meta** · depends-on: [M4.5-EXT-15] · depended-by: -
...
> **tl;dr** · Custom edge-detection stencil marks geometric edges so MSAA shades them at
  sample frequency WITHOUT temporal reprojection/smear. Genuine gap (M4.5-EXT-11 is upscaler-edge only).
...
3. No history buffer, no temporal blend -> no shimmer, no smear.
##### Reference Implementation
```cpp
float edge = depthDisc(dpdx,dpdy) | normalDisc;
if (edge > thr) stencilMask |= EDGE_BIT;
```
##### Player-Facing Impact
...
> **tl;dr** · Subsurface scattering driven by a custom lookup table, NOT brute-force
  volumetric. Genuine gap (M4.5-EXT-19 is Heiligenschein wet-surface only — no SSS LUT).
...
##### How It Works
1. Precompute SSS response into a 2D LUT (NoL x curvature).
2. Skin shader samples LUT instead of multi-sample volumetric.
3. Forward-rendered skin (T7) only — minority screen (~4%).
##### Reference Implementation
```cpp
vec3 SSS(vec3 N, vec3 L, float curv, vec3 scatter){
  return texture(uSSSLut, vec2(max(dot(N,L),0), curv)).rgb * scatter; }
```
##### Player-Facing Impact
