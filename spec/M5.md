...
2. Uses MCTS ($U(s,a) = Q(s,a) + c_{puct} P(s,a) rac{\sqrt{N(s)}}{1 + N(s,a)}$) to evaluate tactical movement routes across the navigational mesh.
...
2. Diffuses scent intensity over time based on wind velocity vectors: $rac{\partial C}{\partial t} = D 
