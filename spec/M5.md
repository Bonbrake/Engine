...
> **M5 quick-index (90 EXT blocks).** Read ONLY the block(s) you need — each is self-contained.
...
> **tags** · m5.3, system
> **tl;dr** · Confidence-Gated Stealth Strikes — M5-EXT-90.
> **ctx** · M5.3-EXT-07 — M5.3 perception confidence-gated takedowns.
> **meta** · depends-on: — · depended-by: -
...
confidenceThreshold hard-coded 0.15 but perception system uses a different scale → strike never/always eligible.
DmgMult not clamped → C slightly negative yields >8x multiplier (overflow).
Eligibility checked at initiation only, not per-tick → zombie becomes aware mid-animation, strike still lands.
