...
##### Reference Implementation
```cpp
float d=median(r,g,b)-0.5f; alpha=smoothstep(0.5-w,0.5+w,d);
```
##### Player-Facing Impact
...
Serializes active alpha/beta distribution weights from the Thompson-Sampling director engine (`[M5-EXT-49]`'s tension model) directly into the persistence disk file stream, so the director's learned evaluation parameters regarding player survival stress are retained cleanly across application boots instead of resetting to baseline on session reload.
...
```cpp
float a = wrap_angle(poi_angle - cam_yaw);
float x = (a/PI)*ribbon_half; // clamp to edge
```
...
```cpp
poi_registry.upsert(id, {pos, icon, label, tier});
```
...
```cpp
if(event.has_object) poi_registry.upsert(pin_from(event));
```
...
```cpp
float lvl = clamp(bus.sum()/THRESH, 0.f, 1.f);
```
...
```cpp
blit(surface_map.sample(player-R, player+R), overlay);
```
...
```cpp
uint8_t s = rested | (owner<<1) | (can_respec<<2);
```
...
```cpp
if(hub.owned) travel_registry.add(hub);
```
