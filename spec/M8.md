...
##### Reference Implementation
```cpp
float C=exp(mu+sigma*randn()); float P=1.f-exp(-k*(1.f-C)*exposure);
```
##### Player-Facing Impact
...
> **meta** · depends-on: [M8-EXT-53], [M8-EXT-62], [M8-EXT-61], [M8-EXT-15] · depended-by: [M8-EXT-60]
...
##### Reference Implementation
`price = base * factionMod * tierMod * supplyMod;` evaluated per transaction against route/cascade state.
...
> **meta** · depends-on: [M8-EXT-61], [M8-EXT-60], [M8-EXT-14] · depended-by: [M8-EXT-14], [M8-EXT-53], [M8-EXT-62]
...
##### Reference Implementation
`wallet -= price;` with `price` from M8-EXT-14; region CPI ticked per day from supply/demand + route state.
...
##### Reference Implementation
Living communities: a guard rotates, a merchant opens shop, a scavenger climbs the shanty stack (M8-EXT-63) to reach home.
...
> **meta** · depends-on: [M8-EXT-18], [M2-EXT-18], [M8-EXT-56] · depended-by: -
...
##### Reference Implementation
`if (HasPrereq(p) && points>=cost(p)) { Unlock(p); points-=cost(p); }`
...
##### Reference Implementation
`xp[s] += gain; if (xp[s] >= thr*lvl) { lvl++; perkPoints++; } xp[s] *= decayIfIdle;`
...
##### Reference Implementation
```cpp
Layout best=GA_Evolve(pop,fitness,50);
```
##### Player-Facing Impact
...
> **meta** · depends-on: [M8-EXT-14], [M8-EXT-15], [M11-EXT-64], [M5-EXT-60] · depended-by: -
##### How It Works
...
> **meta** · depends-on: [M8-EXT-14], [M8-EXT-15], [M8-EXT-59], [M8-EXT-60], [M8-EXT-57] · depended-by: -
##### How It Works
