...
`grep SEH|__try` across `src/` returns only an imgui false-positive — there is **no** `__try/__except` (or C++ try/catch) guard around enkiTS job dispatch today. The fault-isolation invariant in Math is the *target* state, not present behavior; this gap was originally captured in M0-EXT-40 (merged into this block).
#### [M0-EXT-26] Shader-Compile Timeout & Safe-Mode Boot Fallback
