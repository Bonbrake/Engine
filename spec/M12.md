...
3. If packet loss exceeds 2 consecutive frames, Hermite cubic curves extrapolate remote player velocities ($\mathbf{p}_{extrap} = \mathbf{p}_t + \mathbf{v}_t \Delta t + rac{1}{2}\mathbf{a}_t \Delta t^2$) until next server delta snapshot arrives.
