...
##### Reference Implementation
```cpp
float effC=c0*(1.f-0.7f*fatigue); joint->SetDamping(-effC*angVel);
fatigue += (torque>0.6f*tauMax)? kFat*dt : -kRec*dt; fatigue=clamp(fatigue,0.f,1.f);
```
...
#### [M2-EXT-18] Survival Physiology (stamina/hunger)
> **tags** · general
> **tl;dr** · M2.9. Components.h has PerkPoints but no survival loop.
> **meta** · depends-on: - · depended-by: -
> **ctx** · Survival Physiology (stamina/hunger) -- M2.9. Components.h has PerkPoints but no survival loop.
...
##### Reference Implementation
```cpp
float d=clamp(len(T-sh),abs(L1-L2)+1e-3f,L1+L2-1e-3f);
float elbow=acos(clamp((L1*L1+L2*L2-d*d)/(2*L1*L2),-1,1));
```
...
##### Reference Implementation
```cpp
float h=SampleSeededHeight(S,x,z); ApplyEdit(G,crater);
```
...
##### Reference Implementation
```cpp
float clo=1.0f+SumWornClo(); float Q=(33.f-ambientC)/(0.15f+clo*0.155f);
calorieRate=basal+max(0.f,Q-comfortQ);
```
...
Declares the base "Caloric Condition Metrics (BCS). Layered Thermal Insulation" bullet. BCS feeds `[M2-EXT-65]`'s hydration-exertion depletion as its caloric-reserve term; insulation feeds the same system's sweat/heat-loss calculation as the R-value that gates it.
