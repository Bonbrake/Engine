...
> **ctx** · Asynchronous Collision Cooking & Proxy Primitive Swapping Queue -- Extends M2 (Jolt Physics engine setup) and pairs with M4 chunk streaming topologies.
...
> **ctx** · Continuous Narrow-Phase Contact Point Query Collector Cache -- Sits in front of Jolt's narrow-phase query API, deduping redundant raycasts/sweeps fired by multiple systems in the same frame — melee parry ([M2-EXT-29]), AI perception line-of-sight (M5.3), vault ledge-analysis (M2.9), and any SpatialHash-adjacent query that also needs a physical line trace.
...
> **ctx** · Mechanical Muscle Exhaustion Joint-Friction Damping Adder
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Jolt-to-EnTT Double-Precision Transform Remapper -- M2 (Jolt) + M2.6 (EnTT Transform). Each tick maps Jolt's single-precision body transform back to the authoritative double-precision EnTT Transform.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Kinematic Virtual Sweep Tunneling Safeguard -- M2 character controller + M2.6 collision. Clamps fast kinematic steps to a swept result so the body can't pass through thin geometry.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Character Controller / Player Movement -- M2. grep CharacterController|CharacterBody → 0. Jolt has Character but nothing wires it to input.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Animation / Skinning -- M2/M5. grep Animation|SkinnedMesh → 0. bufferDeviceAddress enabled (Device.cpp:61,116) but unused in shaders.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Ballistics / Projectile Resolution -- M2.9. BulletComponent exists but grep projectile|ricochet|penetration → only data fields, no solver.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Destructible = Placeholder Swap Only -- M2/M3. Destructible.h has intact/destroyed handles; grep fracture|Fracture → 0.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Vehicle System -- M2/M9. grep Vehicle|wheel|suspension → 0. M9 turrets have no base.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Hit/Limb Resolution Pipeline -- M2.9. DamageEvent.LimbId (Components.h:20-29) but no raycast-hit→limb mapping; no mitigation producing ResolvedDamageEvent.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Physics Debug Draw (shipping toggle) -- M2. PhysicsDebugRenderer exists but JPH_DEBUG_RENDERER-gated; no shipping toggle.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · No Real Content / Player / Zombie -- M2/M5. Only prefab spawns 3 test entities (one UnregisteredComponent silently dropped). No player, zombie, weapon.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Save/Load Layer -- M2/M7. grep serialize|SaveGame → 0. Plant SaveSchemaVersion+NetworkProtocolVersion (separate), atomic write (tmp→fsync→checksum→rename), user-data dir via AssetPath, crash tag.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · World-Epoch Clock -- M2/M7. grep WorldEpoch|simulated-seconds → 0. Sim time = frameCount/accumulator.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Survival Physiology (stamina/hunger/thirst/encumbrance/rest) -- M2.9. Replaces the prior stub ("Components.h has PerkPoints but no survival loop") with a full physiology model. Feeds M8-EXT-56 (psyche) + M5-EXT-19 (stealth encumbrance) + M7 (save).
...
> **ctx** · Crafting / Recipes / Workbench -- M8. No crafting system.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Skills / Perks / Progression -- M8.7. PerkPoints exists, no system.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Procedural Variation (bio-weight) -- M5.1. No bio-weight/visual variation generator.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Physics Determinism Test -- M2. No physics-determinism CI.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Reload Mechanics & Cancel -- M2.7. No reload-cancel or reload-state machine in code.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Recoil & Spread Model -- M2.7/M2.9. No recoil or spread solver.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Aiming & ADS System -- M2.7. No aim-down-sights or hip/ADS toggle.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Damage Types & Materials -- M2/M8. No per-material damage response.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Body Temperature & Weather Exposure -- M2.9. No temperature system.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Encumbrance & Stamina Drain -- M2.9. No carry-weight system.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Tactical Clear-and-Vent Malfunctions (Rewarding Wear) -- Complements M2.7 core weapon cycling loops, reads BarrelHeat.fouling as written by [M2-EXT-35] Atmospheric Powder-Fouling Friction Accumulation (build/wire both in the same pass — this system has no fouling input to react to otherwise), and reads brand quality tier from data-driven JSON weapon records.
...
> **ctx** · Context-Sensitive Interaction Radial -- Surfaces M2-EXT-24/25 prompts and M8-EXT-09 inventory actions as a radial; lives inside the shell.
> **meta** · depends-on: - · depended-by: -
...
> **ctx** · Persistent Cross-World NPC Registry -- M2/M7. Part 33. Reuses M7-EXT-01 (StableId) + M7-EXT-19 (save compat) + M2-EXT-18 (physiology) + M5.1 (companion AI). Two tiers: persistent (player-made / notable) vs generated (world-filler, respawn on world-gen). Either/or E87 RESOLVED (local-first principle): registry is **local-per-save** — offline play is primary, friends-co-op (M12-EXT-22 peer model) reads the same local registry; NO cloud dependency.
...
> **ctx** · Procedural NPC Detail Generator -- M2. Part 33. Reuses M2-EXT-18 (needs) + M8-EXT-16 (residents) + M8-EXT-56 (psyche) + M8-EXT-62 (stratification) + M4-EXT-92 (seed). DF personality-facet model (51 traits + values + goals + memories) + CK3 courtier opinion/rank. Either/or E88 RESOLVED (compact): 12-facet trait model — covers the behaviorally-salient axes (altruism, greed, bravery, diligence, sociability, stability, ambition, trust, aggression, curiosity, conformity, impulsiveness) at a fraction of DF's 51-facet storage cost; extensible to 51 if profiling shows need.
...
> **ctx** · Immersive Interaction Shell -- M2. Unifies every hands-on interaction in the game under one consistent, in-world presentation so the immersive layer feels coherent. Reuses M2-EXT-48 (radial) + M11 (diegetic HUD) + M9-EXT-30/31/32/33/34. Legacy stub M2-EXT-48 is the radial subset; this block is the superset shell.
...
> **ctx** · Interaction Proficiency -- M2/M8. Makes the immersive layer progress with the player instead of staying flat. Reuses M8-EXT-18 (skill XP) + M9-EXT-30..34 + M2-EXT-78 (shell). The five minigames each map to a skill line in M8-EXT-18.
...
> **ctx** · Limb Damage & Dismemberment -- M2/M5. Melee (M2-EXT-43) + firearms (M2-EXT-40) feed hit impulse + weapon-class into this system. Reuses M5 ragdoll (M5-EXT-19 skinning) + Jolt body. Gore routes to M6.5-EXT-13 / M7-EXT-01 (M-rated, dark).
...
> **ctx** · Bleeding & Wound Treatment -- M2/M8. Complements M8-EXT-03 (SEIR illness) and M2-EXT-18 (survival physiology). Wounds are acute (bleed-out), illness is systemic (SEIR) — distinct tracks.
...
> **ctx** · M2-EXT-101 — M2.6 open-world foundations.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2-EXT-102 — M2.6 open-world foundations.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2-EXT-103 — M2.6 open-world foundations.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2.9-EXT-01 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2.9-EXT-02 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2.9-EXT-03 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2.9-EXT-04 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2.9-EXT-05 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2-EXT-109 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2-EXT-110 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **ctx** · M2-EXT-111 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **tags** · m2.9, system
> **tl;dr** · Sleep-Deprivation Hallucination Drift — M2-EXT-112.
> **ctx** · M2-EXT-112 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
...
> **tags** · m2.9, system
> **tl;dr** · Layered Wet-Clothing Insulation — M2-EXT-113.
> **ctx** · M2-EXT-113 — M2.9 traversal, ballistics & survival physiology.
> **meta** · depends-on: — · depended-by: -
